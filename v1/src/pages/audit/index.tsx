...
import { Card, Row, Col, DatePicker, Button, Tag, Space, Typography, Modal, Progress, App } from 'antd';
import { ReloadOutlined, CheckOutlined, DollarOutlined, BankOutlined, RetweetOutlined } from '@ant-design/icons';
...
const AuditPage: React.FC = () => {
  ...
  const columns: ProColumns<AuditRecord>[] = [
      ...
      render: (_, record: AuditRecord) => {
        ...
        switch (record.txnType) {
          ...
          case 'CANCEL':
            ...
            break;
          default:
            text = TransactionTypeText[record.txnType as keyof typeof TransactionTypeText] || record.txnType;
        }
    ...
    },
    {
      ...
      width: 100,
      render: (_, record: AuditRecord) => {
        const color = record.txnStatus === '1' ? 'success' : 'error';
        const text = record.txnStatus === '1' ? '成功' : '失败';
    ...
    },
    {
      ...
      render: (_, record: AuditRecord) => (
        <span style={{ 
          color: record.finalAmt < 0 ? '#ff4d4f' : '#52c41a',
          fontWeight: 'bold'
        }}>
          {record.finalCcy} {record.finalAmt}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      width: 170,
      render: (_, record: AuditRecord) => {
        return formatBeijingTime(record.txnTime);
      },
      sorter: true,
    },
    {
    ...
    },
    {
      ...
      render: (_, record: AuditRecord) => {
        ...
        // 判断是否已经完成补偿充值（交易类型变为CANCEL或F）
        const isCompensated = record.txnType === 'CANCEL' || record.txnType === 'F';
