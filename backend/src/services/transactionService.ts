...
export class TransactionService {
    ...
    try {
      ...
      const transaction = await prisma.cardTransaction.create({
        data: {
          ...
          txnTime: data.txnTime ? new Date(data.txnTime + 'Z') : new Date(), // 将卡商时间当作UTC时间处理
      ...
      if (data.txnType === 'A' && data.txnStatus === '1' && finalAmount > 0) {
        ...
        // 余额变动日志已移除，仅记录操作日志
        logger.info(`Card ${card.cardId} balance updated: ${card.balance} -> ${card.balance.sub(new Decimal(finalAmount))} (Auth: ${data.txnId})`);
      }
      ...
      if (!amountDiff.isZero()) {
        ...
        if (card) {
          if (amountDiff.gt(0)) {
            ...
            // 余额变动日志已移除，仅记录操作日志
            logger.info(`Settlement adjustment - additional charge: ${amountDiff} for card ${card.cardId} (Auth: ${data.authTxnId}, Settlement: ${data.settleTxnId})`);
          } else {
            // 结算金额小于授权金额，退还差额
            await prisma.virtualCard.update({
              where: { id: card.id },
            ...
            // 余额变动日志已移除，仅记录操作日志
            logger.info(`Settlement adjustment - refund difference: ${amountDiff.abs()} for card ${card.cardId} (Auth: ${data.authTxnId}, Settlement: ${data.settleTxnId})`);
          }
        }
      }
    ...
    try {
      ...
      if (data.txnStatus === '1') {
        ...
        if (txnTypeEnum === CardTxnType.SETTLEMENT && settlementAmount > 0) {
          ...
          // 余额变动日志已移除，仅记录操作日志
          logger.info(`Settlement charge: ${settlementAmount} for card ${card.cardId}, balance: ${balanceBefore} -> ${balanceAfter} (${data.txnId})`);
        } else if (txnTypeEnum === CardTxnType.REFUND && settlementAmount > 0) {
          // 退款 - 增加余额
          const balanceBefore = card.balance;
          const balanceAfter = card.balance.add(new Decimal(settlementAmount));
          ...
          // 余额变动日志已移除，仅记录操作日志
          logger.info(`Refund: ${data.finalAmt} for card ${card.cardId}, balance: ${balanceBefore} -> ${balanceAfter} (${data.txnId})`);
        }
      }
  ...
  async getTransactions(params: TransactionQueryParams): Promise<PaginatedTransactionResponse> {
    ...
    if (cardId) {
      where.cardId = cardId;
      console.log('🔍 查询交易记录 - 过滤条件', { cardId, where });
    }
    ...
    if (startDate || endDate) {
      ...
      if (startDate) {
        // 直接按日期查询，不做时区转换
        // startDate='2025-09-26' -> 查询 2025-09-26T00:00:00Z 及之后
        where.txnTime.gte = new Date(startDate + 'T00:00:00.000Z');
      }
      if (endDate) {
        // 直接按日期查询，不做时区转换
        // endDate='2025-09-26' -> 查询 2025-09-26T23:59:59Z 及之前
    ...
    console.log('📊 查询交易记录结果', { 
      cardId, 
      total, 
      transactionCount: transactions.length,
      transactionIds: transactions.map(t => t.txnId).slice(0, 5)
    });

    const data: TransactionResponse[] = transactions.map(txn => ({
      id: txn.id,
      cardId: txn.cardId,
      cardNo: txn.virtualCard?.cardNo || txn.cardNo, // 优先使用虚拟卡表中的真实卡号
      status: txn.status,
    ...
    try {
      ...
      if (type === 'AUTH') {
        whereCondition.txnType = {
          in: [CardTxnType.AUTH, CardTxnType.SETTLEMENT, CardTxnType.REFUND, CardTxnType.CANCEL] // 授权相关交易类型，不包含授权撤销
        };
      } else if (type === 'SETTLE') {
    ...
    try {
      ...
      const transaction = await prisma.cardTransaction.create({
        data: {
          ...
          txnType: txnTypeEnum,
          txnStatus: data.txnStatus,
          txnTime: new Date(data.txnTime + 'Z'), // 直接存储北京时间，不做时区转换
  ...
  async processCompensationRecharge(txnId: string) {
    ...
    try {
      ...
      // 3. 更新交易记录 - 使用真正的北京时间 (UTC+8)
      const beijingTime = new Date(Date.now() + 8 * 60 * 60 * 1000); // 当前UTC时间 + 8小时 = 北京时间
  ...
  async exportTransactionsToExcel(transactions: any[]): Promise<Buffer> {
    try {
      logger.info('📊 开始导出Excel', { count: transactions.length });
      ...
      const excelData = transactions.map((txn, index) => ({
        ...
        '交易类型': this.getTransactionTypeText(txn.txnType),
   ...
   * 获取交易类型文本
   */
  private getTransactionTypeText(txnType: string): string {
    const typeMap: Record<string, string> = {
      'AUTH': '授权',
      'AUTH_CANCEL': '授权撤销',
      'SETTLEMENT': '结算',
