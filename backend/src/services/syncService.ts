...
export class SyncService {
  ...
  private async processAuthTransaction(
    ...
    // 根据实际API返回的key_list顺序（21个字段）：
    // ["card_id", "txn_id", "txn_type", "txn_status", "txn_time", "txn_amt", "txn_ccy", "bill_amt", "bill_ccy", "mcc", "merch_name", "merch_ctry", "origin_txn_id", "decline_reason", "auth_code", "sub_id", "force_post", "pre_auth", "biz_type", "remark", "wallet_id"]
    ...
    // 记录解析的原始数据用于调试 (生产环境可注释掉)
    if (process.env.NODE_ENV !== 'production') {
      logger.debug('Processing Auth Transaction Raw Data', {
        txnId,
        cardId,
        rawAuthItem: authItem,
        parsedData: {
          cardId, txnId, txnType, txnStatus, txnTime, txnAmt, txnCcy,
          billAmt, billCcy, mcc, merchName, merchCtry, originTxnId, 
          declineReason, authCode, subId, forcePost, preAuth, bizType, remark, walletId
        }
      });
    }
    ...
    const transactionData = {
      ...
      forcePost: forcePost || false,
      preAuth: preAuth || false
    };
